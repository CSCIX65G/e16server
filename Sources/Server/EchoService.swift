...
struct EchoService: Service {
    ...
    static let inputDecoder = { (request: SmokeHTTP1Request) throws -> EchoInput in
        Log.debug("Handling EchoService request: \(request)")
        guard let data = request.body, let decoded = String(data: data, encoding: .utf8) else {
    ...
    static let transform = { (input: EchoInput, context: ApplicationContext, responseHandler: EchoResultHandler) in
    ...
    static let outputEncoder = { (request: SmokeHTTP1Request, output: EchoOutput, responseHandler: HTTP1ResponseHandler) in
        ...
        responseHandler.complete(status: responseCode, responseComponents: response)
    }
    
    static let serviceHandler = OperationHandler<ApplicationContext, SmokeHTTP1Request, HTTP1ResponseHandler>(
